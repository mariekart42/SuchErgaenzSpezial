...
Weiter:
        ...
        For n = 1 To i
            If Not (m = n) Then 'kein Objekt mit sich selbst vergleichen
                If InStr(splitar(m), splitar(n)) > 0 Then
                     MsgBox ("doppelte Verwendung Error -2")
    '                                      MsgBox "Der Suchbegriff " & Chr(34) & splitar(n) & Chr(34) & " wird im Suchbegriff " & Chr(34) & splitar(m) & Chr(34) & " wiederverwendet!", vbOKOnly + vbCritical, "Suchen & Ergänzen fehlgeschlagen!"
                ...
                Else
                    If InStr(splitar(n), splitar(m)) > 0 Then
                     MsgBox ("doppelte Verwendung Error -3")
    ...
    If strVariable <> "" Then
        GoTo SuchErsetz
    Else
        MsgBox ("Datei ist leer, change dat lol")
    End If
...
SuchErsetz:
    ...
    ' Determine the correct replacement based on singular/plural
    Dim replacement As String
    If foundText = suchstring Then
        replacement = foundText & " (" & ergaenzstring & ")"
    ElseIf foundText & "s" = suchstring Then
        replacement = foundText & "s" & " (" & ergaenzstring & ")"
    Else
        ' Handle other cases as needed
        replacement = foundText
    End If

                    MsgBox ("Found this: " & replacement)
