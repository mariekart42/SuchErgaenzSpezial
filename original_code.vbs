...
Weiter:
            ...
            If InStr(profil, "\sh\") = 0 Then
                ...
                Server = "\\brefile11\userhomes$\"
                Pfad = "\Desktop\"
                'TXT-Datei auf lokalem User-Desktop prüfen
                If Dir(Server & user & Pfad & "bezug.txt") <> "" Then
            ...
            Else
                If InStr(profil, "\sh\") <> 0 Then
                    ...
                    Server = "\\brefile01\profile$\"
                    Pfad = "\sh\Desktop\"
                    'TXT-Datei auf SH-User-Desktop prüfen
                    If Dir(Server & user & Pfad & "bezug.txt") <> "" Then
                        Antwort6 = MsgBox(Mldg6, Stil6, Titel6)
                    Else
                        Antwort7 = MsgBox(Mldg7, Stil7, Titel7)
            ...
            'Antwort auswerten
            Select Case Antwort6
                Case vbYes
                    
                    'Datei öffnen und Zeile für Zeile ins Array einlesen
                    Open Server & user & Pfad & "bezug.txt" For Input As #1
                    'ReDim ar(0)
                    Do While Not EOF(1)
